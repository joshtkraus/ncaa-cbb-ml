...
def clean_KP(df):
    ...
    Args:
        df: KenPom DataFrame with a 'Team' column.
    ...
    df.loc[:, "Team"] = df["Team"].str.replace(
        "Mount St. Mary's", "Mount Saint Mary's", regex=False
    )
    df.loc[:, "Team"] = df["Team"].str.replace(r"St\.", "State", regex=True)
    df.loc[:, "Team"] = df["Team"].str.replace(";", "", regex=False)
    df.loc[:, "Team"] = df["Team"].str.replace("Grambling State", "Grambling", regex=False)
    df.loc[:, "Team"] = df["Team"].str.replace(
        "Texas A&M Corpus Chris", "Texas A&M Corpus Christi", regex=False
    )
    ...
    df.loc[:, "Team"] = df["Team"].str.replace(
        "Central Connecticut", "Central Connecticut State", regex=False
    )
    df.loc[:, "Team"] = df["Team"].str.replace("Troy State", "Troy", regex=False)
    df.loc[:, "Team"] = df["Team"].str.replace(r"^NC Asheville", "UNC Asheville", regex=True)
