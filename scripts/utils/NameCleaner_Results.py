...
def clean_results(dict_teams):
    """Apply name normalization to all teams in the results dictionary.

    Args:
        dict_teams: Nested dict of tournament results keyed by year, region, and round.
    ...
    for _year, regions in dict_teams_copy.items():
        for region, rounds in regions.items():
            if region not in ["NCG", "Winner"]:
                ...
                dict_teams_copy[_year][region] = clean_names(dict_teams_copy[_year][region])
