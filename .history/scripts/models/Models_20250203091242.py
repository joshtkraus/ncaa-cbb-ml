def check_hyperparameters(param_grid, tuned_params, param):
    import warnings
    if min(param_grid) == tuned_params[param]:
        warnings.warn('WARNING Hyperparameter minimum reached: ',param)
    elif max(param_grid) == tuned_params[param]:
        warnings.warn('WARNING Hyperparameter maximum reached: ',param)
...
def GB_Fit(team_data, r, best_features=None):
    ...
    # Get Hyperparameters
    if best_features != None:
        max_depth, n_estimators, min_samples_leaf, min_samples_split = gradient_boosting(step='Post')
    else:
        max_depth, n_estimators, min_samples_leaf, min_samples_split = gradient_boosting(step='Pre')
    k_neighbors = smote()
...
def NN_Fit(team_data, r, best_features=None):
    ...
    # Get Hyperparameters
    if best_features != None:
        hidden_layer_size, alpha, activation = neural_network(step='Post')
    else:
        hidden_layer_size, alpha, activation = neural_network(step='Pre')
    k_neighbors = smote()
